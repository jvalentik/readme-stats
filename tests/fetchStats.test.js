...
describe("Test fetchStats", () => {
  it("should fetch correct stats", async () => {
    let stats = await fetchStats("jvalentik");
  ...
  it("should throw error", async () => {
    ...
    await expect(fetchStats("jvalentik")).rejects.toThrow(
      "Could not resolve to a User with the login of 'noname'.",
    );
  });

  it("should fetch and add private contributions", async () => {
    let stats = await fetchStats("jvalentik", true);
    const rank = calculateRank({
      totalCommits: 150,
      totalRepos: 5,
      followers: 100,
  ...
  it("should fetch total commits", async () => {
    mock
      .onGet("https://api.github.com/search/commits?q=author:jvalentik")
      .reply(200, { total_count: 1000 });

    let stats = await fetchStats("jvalentik", true, true);
    const rank = calculateRank({
      totalCommits: 1050,
      totalRepos: 5,
      followers: 100,
  ...
  it("should exclude stars of the `test-repo-1` repository", async () => {
    ...
    let stats = await fetchStats("jvalentik", true, true, ["test-repo-1"]);
    const rank = calculateRank({
      totalCommits: 1050,
      totalRepos: 5,
      followers: 100,
